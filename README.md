> [!IMPORTANT]
> For v2.0 and later, you need atleast python 3.10 installed.  
> For v1.4.1 and earlier, you need atleast python 1.0 installed.

> [!NOTE]
> v2.0 out now!

This is a collection of random python files that I have made

## These are the python files that are currently in the collection
### NumberGenerators.py (in v2.0)
- Prime number generator - generates all prime numbers in a range that you specify (in v1.4.1, separated into primeNumberGenerator.py)
- Exponent generator - generates all numbers in a range you that specify that can be gotten by raising an integer to the power that you specify (As of v2.0) (in v1.4.1, split into cubeNumberGenerator.py and squareNumberGenerator.py)
- Number factors generator - generates a list of all of the factors of a number (in v1.4.1, separated into numberFactorsGenerator.py)

### Others
- Rock paper scissors - play rock paper scissors against a computer
- Number guessing game - guess a random number
- Unit converter - converts between various units (in v2.1)
- Coin flip - flip a specified number of coins (in v2.1)
- Dice roll - roll a specified number of dice with a specified number of faces (in v2.1)
